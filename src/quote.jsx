...
function VQuote() {
  ...
  return (
    <div className="v-scope" style={{ background:'var(--paper)', width:'100%', minHeight:'100%' }}>
      <VNav active="Pricing" />

      <div style={{ display:'grid', gridTemplateColumns:'1.3fr 1fr', minHeight: 900 }}>
        {/* Left: form */}
            ...
            {steps.map((label, i) => (
              <React.Fragment key={label}>
                <button onClick={() => i < step && setStep(i)} style={{
                  background:'transparent', border:0, padding:0, cursor: i < step ? 'pointer' : 'default',
                  display:'flex', alignItems:'center', gap: 10,
                }}>
                  <span style={{
                    width: 24, height: 24, borderRadius: 999,
                    border: `1px solid ${i <= step ? 'var(--brand)' : 'var(--line)'}`,
                    background: i < step ? 'var(--brand)' : i === step ? 'var(--brand)' : 'transparent',
                ...
                {i < steps.length - 1 && <span style={{ flex: 1, height: 1, background: i < step ? 'var(--brand)' : 'var(--line)', margin:'0 14px' }} />}
              </React.Fragment>
            ))}
          </div>
          ...
          <div style={{ minHeight: 520 }}>
                  ...
                  {services.map(s => {
                    ...
                    return (
                      <button key={s.id} onClick={() => setService(s.id)} style={{
                        textAlign:'left', padding: 18, background: selected ? '#fff' : 'transparent',
            ...
            {step === 1 && (
              <div>
                ...
                <Field label={`Number of ${selSvc.unit}s`}>
                  <div style={{ display:'flex', alignItems:'center', gap: 16 }}>
                    <input type="range" min="1" max="50" value={pages} onChange={e => setPages(+e.target.value)} style={{ flex: 1 }} />
                    <div style={{
                      width: 80, padding:'10px 14px', border:'1px solid var(--line)', borderRadius: 4,
                      ...
                      <div style={{ display:'flex', flexWrap:'wrap', gap: 8 }}>
                        {['USCIS','Court','University','Employer','Consulate','Other'].map(p => (
                          <button key={p} onClick={() => setPurpose(p)} style={{
                            padding:'9px 14px', fontSize: 13, border:`1px solid ${purpose === p ? 'var(--brand)' : 'var(--line)'}`,
                ...
                <div style={{ marginTop: 32, display:'flex', flexDirection:'column', gap: 12 }}>
                  <Toggle checked={rush} onChange={setRush}
                    title="Same-day rush"
                    desc="Delivered in 6 hours or less. +50% on base price." />
                  {service === 'certified' && (
                    <Toggle checked={notarize} onChange={setNotarize}
              ...
              <div>
                ...
                {!uploaded ? (
                  ...
                  <div style={{ padding: 20, background:'#fff', border:'1px solid var(--line)', borderRadius: 4, display:'flex', alignItems:'center', gap: 16 }}>
                    <div style={{ width: 48, height: 60, border:'1px solid var(--line)', background:'var(--paper)', flex:'0 0 auto', display:'flex', alignItems:'center', justifyContent:'center' }}>
            ...
            {step === 3 && (
              <div>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap: 18 }}>
                  ...
                  <Field label="Notes for your linguist (optional)">
              ...
              <div>
                ...
                <div style={{ padding: 28, border:'1px solid var(--line)', background:'#fff', borderRadius: 4 }}>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap: 8, fontSize: 13.5 }}>
                    <Line k={`Base · ${pages} × $${selSvc.price.toFixed(2)}`} v={`$${base.toFixed(2)}`} />
                    {rush && <Line k="Same-day rush · +50%" v={`$${rushFee.toFixed(2)}`} />}
                    {notarizeFee > 0 && <Line k="Notarized hard copy" v={`$${notarizeFee.toFixed(2)}`} />}
                    <Line k="Estimated delivery" v={rush ? 'Tonight, 10:47 PM' : 'Tomorrow, 3:15 PM'} />
                  </div>
                </div>
                ...
                <div style={{ marginTop: 28, padding: 16, background:'var(--warm)', borderRadius: 4, fontSize: 13, color:'rgba(10,10,10,0.75)', lineHeight: 1.55, display:'flex', gap: 12 }}>
                  <div style={{ width: 24, height: 24, flex:'0 0 auto', borderRadius: 999, border:'1.5px solid var(--brand)', color:'var(--brand)', display:'flex', alignItems:'center', justifyContent:'center', fontSize: 12, fontFamily:'Source Serif 4' }}>V</div>
                  <div>
                    Alexis Ortega (ATA-certified, 9 years, specializes in Latin American civil documents) is available to start this translation. If you pay in the next 30 minutes, she begins today.
                  </div>
                </div>
              </div>
            )}
          ...
          {step < 4 && (
            <div style={{ display:'flex', justifyContent:'space-between', marginTop: 48, paddingTop: 24, borderTop:'1px solid var(--line)' }}>
              <button onClick={prev} disabled={step === 0} style={{
                background:'transparent', border:0, color: step === 0 ? 'rgba(10,10,10,0.25)' : 'var(--mute)',
        ...
        {/* Right: live summary */}
        <div style={{ padding: 56, background:'#fff', position:'relative' }}>
          <div style={{ position:'sticky', top: 80 }}>
            ...
            <div style={{ fontSize: 13, color:'var(--mute)', marginBottom: 32 }}>
              Firm price. Updated as you change selections.
            </div>

            <hr className="v-hr" />
            <div style={{ display:'flex', flexDirection:'column', gap: 14, margin:'24px 0' }}>
              <SummaryRow label="Service" value={selSvc.name} />
              <SummaryRow label="Direction" value={`${fromLang} → ${toLang}`} />
              <SummaryRow label="Volume" value={`${pages} ${selSvc.unit}${pages > 1 ? 's' : ''}`} />
              {service === 'certified' && <SummaryRow label="Purpose" value={purpose} />}
              <SummaryRow label="Rush" value={rush ? 'Same-day' : 'Standard · 24h'} />
              {service === 'certified' && <SummaryRow label="Hard copy" value={notarize ? 'Notarized' : 'Digital only'} />}
            </div>
            <hr className="v-hr" />
            ...
            <div style={{ marginTop: 32, padding: 20, background:'var(--paper)', border:'1px solid var(--line)', borderRadius: 4 }}>
              ...
              <div style={{ fontSize: 12, color:'var(--mute)', marginTop: 14, display:'flex', justifyContent:'space-between' }}>
                ...
                <span style={{ display:'flex', gap: 2 }}>
                  {[0,1,2,3,4].map(i => <span key={i} style={{ color:'#00B67A' }}>★</span>)}
                </span>
              </div>
            </div>
...
function Toggle({ checked, onChange, title, desc }) {
  return (
    <button onClick={() => onChange(!checked)} style={{
      ...
      padding:'16px 18px', background: checked ? '#fff' : 'transparent',
      ...
      <div style={{
        width: 44, height: 24, borderRadius: 999, background: checked ? 'var(--brand)' : 'rgba(10,10,10,0.15)',
        position:'relative', transition:'background .12s',
      }}>
        <div style={{
          position:'absolute', top: 2, left: checked ? 22 : 2, width: 20, height: 20, borderRadius: 999,
          background:'#fff', boxShadow:'0 1px 3px rgba(0,0,0,0.15)', transition:'left .15s',
        }} />
      </div>
    </button>
  );
